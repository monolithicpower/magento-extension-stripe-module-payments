...
class Subscriptions
{
    ...
    public function confirm($subscriptionId, $profile, $metadata, $order)
    {
        ...
        // If there was a payment upon subscription creation (there might not be with trial subscriptions and $0 initial fees/shipping)
        // then save the invoice number and create it after the order is placed
        $payment = $order->getPayment();
        if (!empty($subscription->latest_invoice))
            $payment->setAdditionalInformation("initial_subscription_invoice", $subscription->latest_invoice->id);
